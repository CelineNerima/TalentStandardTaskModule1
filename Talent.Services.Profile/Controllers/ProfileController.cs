...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> UpdateTalentProfile(TalentProfileViewModel model, string updaterId)
        {
            //Your code here;
            ...
            try
            {
                ...
                // Update the talent profile using the profile service
                var success = await _profileService.UpdateTalentProfile(model, userId);
                ...
                {
                    // Return an error response if the update was not successful
            ...
            catch (Exception e)
            {
                ...
                return Json(new { Success = false, Message = "An error occurred while updating the profile", ErrorDetails = e.ToString() });
            }
        }
        ...
        public async Task<IActionResult> GetSkills()
        {
            ...
            try
            {
                ...
                // Call the new method from ProfileService to get skills
                var skills = await _profileService.GetSkills(userId);
        ...
        public ActionResult AddSkill([FromBody] AddSkillViewModel skill)
        {
            ...
            try
            {
                if (_profileService.AddNewSkill(skill))
                {
                    return Json(new { Success = true });
                }
        ...
        public async Task<ActionResult> UpdateProfilePhoto([FromForm] IFormFile file)
        {
            try
            {
                ...
                if (success)
                {
                    var imageUrl = await _fileService.GetFileURL(talentId, FileType.ProfilePhoto);
                    return Json(new { success = true, profilePath = imageUrl });
                }

                // If the update was not successful, return a detailed error message
                return Json(new { success = false, message = "Failed to update profile photo. Please try again." });
            }
            catch (Exception e)
            {
                // Log the detailed error
                Console.WriteLine($"Error in UpdateProfilePhoto: {e.Message}");

                // Return a detailed error message to the client
                return Json(new { success = false, message = "Failed to update profile photo. Please try again.", detailedError = e.ToString() });
            }
